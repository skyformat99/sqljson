...
static void
checkJsonOutputFormat(JsonFormat *format, Oid targettype)
{
...
transformJsonOutput(ParseState *pstate, JsonOutput **poutput)
{
	...
	if (output->format.type == JS_FORMAT_DEFAULT)
		...
		checkJsonOutputFormat(&output->format, output->typid);
}
					 ...
					 Oid aggfnoid, Oid aggtype)
{
	...
	aggref->aggfilter = agg_ctor->agg_filter ? (Expr *)
	...
	return coerceJsonFuncExpr(pstate, (Node *) aggref, agg_ctor->output);
}
...
transformJsonIsPredicate(ParseState *pstate, JsonIsPredicate *pred)
{
	...
	else
	{
		...
		if (pred->format.type == JS_FORMAT_JSONB)
			ereport(ERROR,
					(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
					 /* FIXME errposition(pred->format.pos), */
