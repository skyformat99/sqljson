...
select _jsonpath_predicate('[]', '$[*]');
select _jsonpath_predicate('[[1, true], [2, false]]', 'strict $[*] ? (@[0] > $x) [1]', '{"x": 1}');
select _jsonpath_predicate('[[1, true], [2, false]]', 'strict $[*] ? (@[0] < $x) [1]', '{"x": 2}');
...
select _jsonpath_object('[1, 2, 3]', 'strict ($[*].a > 3).type()');
...
select _jsonpath_object('[]', 'strict $.datetime()');
select _jsonpath_object('{}', '$.datetime()');
select _jsonpath_object('""', '$.datetime()');
